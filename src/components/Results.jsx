...
export const Results = () => {
  ...
  switch (location.pathname) {
    case "/search":
      return (
        <div className="flex flex-wrap justify-between gap-y-10 sm:px-56">
          {results?.map(({ link, title }, index) => (
            <div key={index} className="md:w-2/5 w-full">
              <a href={link} target="_blank" rel="noreferrer">
                <p className="text-sm">
                  {link.length > 30
                    ? link.substring(0, 30).concat("...")
                    : link}
                </p>
    ...
    case "/news":
      return (
        <div className="grid justify-between gap-y-10 sm:px-56 items-center">
